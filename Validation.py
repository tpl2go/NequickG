...
def highsolaractivity():
    ...
    with open('./Validation/High_reference.dat') as infile:
        with open('./Validation/High_output2.dat', 'w') as outfile:
            ...
            for row in reader:
                ...
                row[8] = NEQ_global.sTEC2(h1, lat1, lon1, h2, lat2, lon2) / 10**16
                writer.writerow(row)
highsolaractivity()
...
def lowsolaractivity():
    ...
    with open('./Validation/Low_reference.dat') as infile:
        with open('./Validation/Low_output2.dat', 'w') as outfile:
            writer = csv.writer(outfile, delimiter = ' ')
            reader = csv.reader(infile, delimiter = ' ')
            for row in reader:
                row = map(float, row)
